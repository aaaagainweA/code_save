...
def train_test(path,model_no,key,target,cols,flag=0):
    train=pd.read_csv(path+'data/output/ptrain_'+model_no+'.csv',encoding='gbk')
    train[target[0]][train[target[0]]==2]=0
    x=train[cols]
    y=train[target[0]]
    ...
    return xt,yt, x_train,x_test,y_train,y_test,train
...
def rfc_model(x,y,x_train,x_test,y_train,y_test,n,m):
    rfc=RandomForestClassifier(n_estimators=n,max_features=0.6,max_depth=m,oob_score=True,n_jobs=16)
    rfc.fit(x,y)
    return rfc

def model_run(model_no,path):
    pd.set_option('display.max_rows', 500)#设置显示最大行数
    #path='D:/rabit/1008/'
    #model_no='czr_1015_2'
    print(model_no)
    ...
    x,y,x_train,x_test,y_train,y_test,train=train_test(path,model_no,key,target,cols_corr)  #数据获取  
    #逻辑回归模型调参
    #penalty='l1'
    #c=0.001

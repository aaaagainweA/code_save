...
# order_card.dropna(subset='recommend_store_id',inplace=True)
...
# recharge_up.to_excel('充值升单.xlsx')
...
# df_user_promote0.to_excel('升单订单金额.xlsx')
...
# df_user_repurchase0.to_excel('升单充值金额.xlsx')
...
# card_up_sum3=pd.merge(order_card['user_id'],card_user_up,on='user_id',how='inner')
...
# repurchase_sum1.to_excel('老客到店人数.xlsx')
...
repurchase_sum2.drop_duplicates(subset=['user_id'], inplace=True)
# repurchase_sum2.to_excel('新客到店人数.xlsx')
...
# repurchase_people.to_excel('复购成交人数.xlsx')
...
# plan_money.to_excel('自拓客订单金额.xlsx')
...
# plan_money1.to_excel('自拓客充值金额.xlsx')
...
df_shop=pd.read_sql(sql1,db)

st=int(float(begin_date))*1000
ed=int(float(end_date))*1000


df_shop=df_shop.loc[(df_shop['到店时间']<ed)&(df_shop['到店时间']>=st),['到店门店','手机号']]
df_lll1=df_shop.groupby('到店门店').count().reset_index()
df_lll1.rename(columns={df_lll1.columns[0]:'门店名称',df_lll1.columns[1]:'到店人数'},inplace=True)
df_lll1['实时业绩目标']=df_lll1['到店人数']*3000


df_result = pd.merge(df_result, df_lll1, left_on='store_name', right_on='门店名称', how='left')
df_result.drop('门店名称', axis=1, inplace=True)

def extract_city(x: str):
    if str(x) != 'nan':
        y = x[:2]
        return y
df_result['城市']=df_result['城市'].apply(lambda x: extract_city(x))

df_sold=df_result.loc[:,['城市','售卡人数']]
df_sold.fillna(0, inplace=True)
df_sold=df_sold.groupby('城市').sum().reset_index()

df_ccc=pd.read_excel(r'D:/python_project/projects/region_adverting/city_consume.xlsx')

df_ccc=df_ccc.astype({'进线人数':'int'})
city_inline=df_ccc.loc[:,['城市','进线人数']]

df_turn=pd.merge(city_inline,df_sold,on='城市',how='outer')

df_turn.rename(columns={df_turn.columns[1]:'进线数',df_turn.columns[2]:'售卡总人数'},inplace=True)

df_result=pd.merge(df_result,df_turn,on='城市',how='left')
...
if len(df_refund) != 0 or len(df_back) != 0:
    ...
    升单金额占比 = 升单金额/总成交金额
    进线转化率=售卡总人数/进线数
    目标完成进度=总成交金额/实时业绩目标""", inplace=True)
else:
    df_result.eval("""
        退款率 = 0
        总退款金额=0
...
#             ['大区', '城市','门店名称','实时业绩目标','进线数', '总成交人数', '总成交金额', '总退款金额',
#              '体验卡退款金额', '体验卡退款数量', '实际成交金额', '退款率', '充值人数', '充值金额', '充值占比',
#              '商品购买人数',
#              '商品订单金额', '商品订单金额占比',
#              '服务购买人数', '服务订单金额', '服务订单金额占比', '售卡人数', '售卡金额', '售卡金额占比', '新客到店人数',
#              '升单人数', '升单金额', '升单客单价', '升单金额占比', '升单率',
#              '老客到店人数', '复购成交人数', '复购成交金额', '复购客单价', '复购金额占比', '复购率', '服务人数',
#              '服务金额', '自拓客到店人数', '自拓客成交人数',
#              '自拓客实收金额', '自拓客金额占比']]

df_result.sort_values(["大区",'城市'], inplace=True)
...
concat_data1 = concat_data.loc[:,
               ['序号', '大区','城市', '门店名称','实时业绩目标','进线数','售卡总人数','进线转化率', '售卡人数', '售卡金额', '售卡金额占比',
                '新客到店人数', '升单人数', '升单金额', '升单客单价', '升单率', '升单金额占比', '老客到店人数',
                '复购成交人数',
                '复购成交金额', '复购客单价', '复购率', '复购金额占比', '自拓客到店人数', '自拓客成交人数',
                '自拓客实收金额', '自拓客金额占比', '商品购买人数',
                '商品订单金额', '商品订单金额占比', '总成交人数', '总成交金额', '总退款金额', '实际成交金额',
                '服务人数', '服务金额', '退款率','目标完成进度']]
...
concat_data1.to_excel('./门店业绩报表单日' + date + '.xlsx', index=False)

df_area = pd.read_excel('区域城市对照表.xlsx')


df_concat = pd.merge(df_ccc, df_area, on='城市', how='right')

df_concat1 = df_concat.loc[:, ['大区', '广告总额']]
df_concat1 = df_concat1.groupby('大区').sum()

adv_amount_dict = df_concat1.to_dict()
area_amount = adv_amount_dict['广告总额']

total_adv_amount = sum(area_amount.values())

df_concat2=df_concat.loc[:,['大区','进线人数']]
df_concat2 = df_concat2.groupby('大区').sum()
inline_dict = df_concat2.to_dict()
area_inline = inline_dict['进线人数']
total_inline_amount = sum(area_inline.values())



wb = load_workbook(filename='./门店业绩报表单日' + date + '.xlsx')
ws = wb.active
...
# fill9=PatternFill('solid', fgColor='669999')
fill10 = PatternFill('solid', fgColor='FFFF00')  # 黄色
fill8=PatternFill('solid', fgColor='1E90FF')#蓝色1
...
sty13 = NamedStyle(name='sty13', font=font1,border=border1, fill=fill8,alignment=alignment)
...
def get_cal(row_n):
    wb.guess_types = True
    # 总成交金额
    total_amount = ws.cell(row=row_n, column=32).value
    # 退款率计算  退款率 = 总退款金额 / 总成交金额
    total_refund = ws.cell(row=row_n, column=33).value
    refund_rate = total_refund / total_amount
    ws.cell(row=row_n, column=37, value=refund_rate)
    # 商品订单金额占比计算 商品订单金额占比 = 商品订单金额/总成交金额
    order_amount = ws.cell(row=row_n, column=29).value
    order_proportion = order_amount / total_amount
    ws.cell(row=row_n, column=30, value=order_proportion)
    # 售卡金额占比计算 售卡金额占比 = 售卡金额 / 总成交金额
    cardsales_amount = ws.cell(row=row_n, column=10).value
    cardsales_proportion = cardsales_amount / total_amount
    ws.cell(row=row_n, column=11, value=cardsales_proportion)
    # 升单金额占比计算  升单金额占比 = 升单金额/总成交金额
    upgrade_order_amount = ws.cell(row=row_n, column=14).value
    upgrade_order_proportion = upgrade_order_amount / total_amount
    ws.cell(row=row_n, column=17, value=upgrade_order_proportion)
    # 升单率计算  升单率 = 升单人数/新客到店人数
    upgrade_order_num = ws.cell(row=row_n, column=13).value
    new_people_num = ws.cell(row=row_n, column=12).value
    if new_people_num == 0:
        upgrade_order_rate = 0
    else:
        upgrade_order_rate = np.divide(upgrade_order_num, new_people_num)
    ws.cell(row=row_n, column=16, value=upgrade_order_rate)
    # 升单客单价=升单金额/升单人数
    if upgrade_order_num == 0:
        upgrade_order_persales = 0
    else:
        upgrade_order_persales = np.divide(upgrade_order_amount, upgrade_order_num)
    ws.cell(row=row_n, column=15, value=upgrade_order_persales)
    # 复购金额占比计算  复购金额占比 = 复购成交金额/总成交金额
    repurchase_amount = ws.cell(row=row_n, column=20).value
    repurchase_proportion = repurchase_amount / total_amount
    ws.cell(row=row_n, column=23, value=repurchase_proportion)
    # 复购客单价=复购成交金额/复购成交人数
    repurchase_done_num = ws.cell(row=row_n, column=19).value
    if repurchase_done_num == 0:
        repurchase_persales = 0
    else:
        repurchase_persales = np.divide(repurchase_amount, repurchase_done_num)
    ws.cell(row=row_n, column=21, value=repurchase_persales)
    # 复购率=复购成交人数/老客到店人数
    old_custermer_num = ws.cell(row=row_n, column=18).value
    if old_custermer_num == 0:
        repurchase_rate = 0
    else:
        repurchase_rate = np.divide(repurchase_done_num, old_custermer_num)
    ws.cell(row=row_n, column=22, value=repurchase_rate)
    # 自拓客金额占比计算 自拓客金额占比=自拓客实收金额/总成交金额
    tuoke_amount = ws.cell(row=row_n, column=26).value
    tuoke_proportion = tuoke_amount / total_amount
    ws.cell(row=row_n, column=27, value=tuoke_proportion)
    #目标完成进度
    done_per = ws.cell(row=row_n, column=5).value
    if done_per==0:
        ami_done=0
    else:
        ami_done=total_amount/done_per
    ws.cell(row=row_n, column=38, value=ami_done)
    #售卡总人数
    sold_aa = ws.cell(row=row_n, column=9).value
    ws.cell(row=row_n, column=7, value=sold_aa)


# 总计计算
sum_list = [ 5,  9, 10, 12,13,14, 18,19,20, 24,25, 26, 28, 29, 31, 32, 33, 34,35,36]
...
total_amount1 = ws.cell(row=last_row, column=32).value
# 投产比
...
ws.cell(row=last_row, column=6, value=total_inline_amount)
inline_dd=ws.cell(row=last_row, column=6).value
sold_c = ws.cell(row=last_row, column=7).value
if inline_dd == 0:
    inline_dd_ratio = 0
else:
    inline_dd_ratio = sold_c / inline_dd
ws.cell(row=last_row, column=8, value=inline_dd_ratio)
...
for k in range(len(place_list1)):
    ...
    if place_list1[k] != '平台':
        ...
        total_amount2 = ws.cell(row=result, column=32).value
        # 投产比
        ...
        inline_p=area_inline[place_list1[k]]
        ws.cell(row=result, column=6, value=inline_p)
        sold_b = ws.cell(row=result, column=9).value
        inline_n = ws.cell(row=result, column=6).value
        if inline_n == 0 or sold_b==np.nan or inline_n==np.nan:
            inline_n_ratio = 0
        else:
            inline_n_ratio=sold_b/inline_n
        ws.cell(row=result, column=8, value=inline_n_ratio)
...
for s in range(len(place_list1)):
    ...
    # 合并合计单元格
    ws.merge_cells(start_row=res1, start_column=1, end_row=res1, end_column=4)
...
ws.merge_cells(start_row=final_row, start_column=1, end_row=final_row, end_column=4)
...
ws.cell(1, 1).value = '全国市场门店' + date + '销售日报表'
ws.cell(1, 1).style = sty12

# 产品
ws.cell(3, 28).style = sty7
ws.cell(3, 29).style = sty7
ws.cell(3, 30).style = sty7
# 售卡
ws.cell(3, 9).style = sty3
ws.cell(3, 10).style = sty3
ws.cell(3, 11).style = sty3
# 新客业绩
ws.cell(3, 16).style = sty4
ws.cell(3, 17).style = sty4
ws.cell(3, 12).style = sty4
ws.cell(3, 13).style = sty4
ws.cell(3, 14).style = sty4
ws.cell(3, 15).style = sty4
# 老客户复购业绩
ws.cell(3, 22).style = sty5
ws.cell(3, 23).style = sty5
ws.cell(3, 18).style = sty5
ws.cell(3, 19).style = sty5
ws.cell(3, 20).style = sty5
ws.cell(3, 21).style = sty5
# 拓客
ws.cell(3, 26).style = sty6
ws.cell(3, 27).style = sty6
...
# 调整列宽
ws.column_dimensions['D'].width = 42.82
ws.column_dimensions['E'].width = 11.82
ws.column_dimensions['F'].width = 11.82
ws.column_dimensions['G'].width = 11.82
ws.column_dimensions['H'].width = 11.82
ws.column_dimensions['K'].width = 11.27
ws.column_dimensions['L'].width = 11.27
ws.column_dimensions['O'].width = 11.27
ws.column_dimensions['Q'].width = 11.27
ws.column_dimensions['R'].width = 11.27
ws.column_dimensions['S'].width = 11.27
ws.column_dimensions['T'].width = 11.27
ws.column_dimensions['U'].width = 11.27
ws.column_dimensions['W'].width = 11.27
ws.column_dimensions['X'].width = 12.48
ws.column_dimensions['Y'].width = 12.48
ws.column_dimensions['Z'].width = 12.48
ws.column_dimensions['AA'].width =12.48

ws.column_dimensions['AB'].width = 11.27
ws.column_dimensions['AC'].width =11.27
ws.column_dimensions['AD'].width =12.48

ws.column_dimensions['AE'].width = 11.00
ws.column_dimensions['AF'].width = 11.00
ws.column_dimensions['AG'].width = 11.00

ws.column_dimensions['AH'].width =11.27
ws.column_dimensions['AJ'].width = 10.38
...
c_list = []
prcTuple = tuple(prc)
for idy in range(2, 3):  # 遍历A、B、C、D列  #(0,len(prjTuple)) 遍历全部列
    for cell in prcTuple[idy]:
        c_list.append(cell.value)
print(c_list)
c_list1 = []
for j in c_list:
    if j not in c_list1 and j !=0 and j !='城市' and j is not None:
        c_list1.append(j)
print(c_list1)

for c in c_list1:
    python_indices  = [index for (index, item) in enumerate(c_list) if item == c]

    max_a=max(python_indices)
    min_a=min(python_indices)
    if min_a!=max_a:
        value_ll = ws.cell(row=min_a+1, column=3).value
        ws.merge_cells(start_row=min_a+1, start_column=3, end_row=max_a+1, end_column=3)
        ws.cell(min_a+1, 3).value = value_ll
        ws.cell(min_a+1, 3).style = sty0

        value_ll1 = ws.cell(row=min_a+1, column=6).value
        ws.merge_cells(start_row=min_a+1, start_column=6, end_row=max_a+1, end_column=6)
        ws.cell(min_a+1, 6).value = value_ll1
        ws.cell(min_a+1, 6).style = sty0

        value_ll2 = ws.cell(row=min_a+1, column=7).value
        ws.merge_cells(start_row=min_a+1, start_column=7, end_row=max_a+1, end_column=7)
        ws.cell(min_a+1, 7).value = value_ll2
        ws.cell(min_a+1, 7).style = sty0

        value_ll3 = ws.cell(row=min_a+1, column=8).value
        ws.merge_cells(start_row=min_a+1, start_column=8, end_row=max_a+1, end_column=8)
        ws.cell(min_a+1, 8).value = value_ll3
        ws.cell(min_a+1, 8).style = sty0
...
wb.save(filename='./门店业绩报表单日' + date + '.xlsx')
...
wx_api_key = '520d783b-2012-4b32-98c6-b35fcb3b3656'

...
df_ccc=pd.read_excel(r'各城市广告费累计.xlsx')

df_ccc=df_ccc.astype({'进线人数':'int'})
city_inline=df_ccc.loc[:,['城市','进线人数']]
city_inline=city_inline.groupby('城市').sum().reset_index()
...
concat_dfff=concat_data.loc[:,
               ['序号', '大区','城市', '门店名称','到店人数','实时业绩目标','进线数','售卡总人数','进线转化率', '售卡人数', '售卡金额', '售卡金额占比',
...
# concat_dfff.to_excel('实时业绩新客到店人数.xlsx',index=False)
...
concat_data1.to_excel( './门店业绩报表月度累计'+date+'.xlsx', index=False)
...
total_area_adv1=total_area_adv.loc[:,['大区','广告总额']]

total_area_adv1=total_area_adv1.groupby('大区').sum()

adv_amount_dict=total_area_adv1.to_dict()
area_amount=adv_amount_dict['广告总额']

#进线人数
total_city_in=df_concat_advinfo.loc[df_concat_advinfo['日期'].str.contains(month1),['城市','进线人数']]
total_inline=total_city_in.groupby('城市').sum().reset_index()
total_in_amount=total_inline['进线人数'].sum()

total_area_in=pd.merge(total_inline,df_area,on='城市',how='left')

df_concat2=total_area_in.loc[:,['大区','进线人数']]
...
wb = load_workbook(filename='./门店业绩报表月度累计'+date+'.xlsx')
ws = wb.active
...
ws.cell(1, 1).value = '全国市场门店' + date + '累计销售日报表'
...
wb.save(filename = './门店业绩报表月度累计'+date+'.xlsx')

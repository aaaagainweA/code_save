...
df_serve1 = pd.DataFrame(list(col_serve1.find({'create_time': {'$gt': begin_date, '$lt': end_date}},{'_id': 0, 'user_id': 1, 'store_id': 1, 'create_time': 1})))
...
df_result.to_excel( '全国市场门店销售日报表临时用.xlsx', index=False, header=True)
...
concat_data1=concat_data.loc[:,['序号','大区','门店名称','责任目标','冲刺目标','超越目标','售卡人数','售卡金额','售卡金额占比','新客到店人数','升单人数','升单金额','升单客单价','升单率','升单金额占比','老客到店人数', '复购成交人数',
      ...
      '商品订单金额', '商品订单金额占比','总成交人数', '总成交金额', '总退款金额','实际成交金额','服务人数', '服务金额','退款率' ]]
...
concat_data1.to_excel( './全国市场门店销售报表单日'+date+'.xlsx', index=False)


df_area=pd.read_excel('区域城市对照表.xlsx')
df_city_consume=pd.read_excel(r'D:/python_project/projects/region_adverting/city_consume.xlsx')
df_city_consume=df_city_consume.loc[:,['城市','广告总额']]

df_concat=pd.merge(df_city_consume,df_area,on='城市',how='right')
df_concat=df_concat.loc[:,['大区','广告总额']]

df_concat=df_concat.groupby('大区').sum()

adv_amount_dict=df_concat.to_dict()
area_amount=adv_amount_dict['广告总额']

total_adv_amount=sum(area_amount.values())

wb = load_workbook(filename='./全国市场门店销售报表单日'+date+'.xlsx')
ws = wb.active
...
get_cal(last_row)
ws.cell(row=last_row, column=36, value=total_adv_amount)
...
ws.cell(row=last_row, column=37, value=production_ratio)
...
for k in range(len(place_list1)):
    ...
    if place_list1[k]!='平台':
        ...
        ws.cell(row=result, column=36, value=area_adv_amount)
        ...
        ws.cell(row=result, column=37, value=production_ratio)
...
ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=37)
...
test_report = './全国市场门店销售报表单日'+date+'.xlsx'
...
os.remove('./全国市场门店销售报表单日' + date + '.xlsx')
print('删除文件成功')
conn.close()

...
if __name__ == '__main__':
    ...
    result.to_excel('last7days.xlsx', index=False)
    ...
    yesterday_end=str(today) + ' 00:00:00'
    df_yesterday=result.loc[(result['线索创建时间'] >= yesterday_start) & (result['线索创建时间'] < yesterday_end),:]
    df_yesterday.to_excel(f'D:/导线数据/{str(yesterday)}(0-24).xlsx',index=False)
